...
const prevButtom = document.querySelector(`#prev-button`);
...
function updateImage() {
  if (WEB_TECH_IMAGES.length > 0) {
    imageElement.src = WEB_TECH_IMAGES[currentIndex];
  }
}

//Функия для обновления индекса
function updateIndex(increment) {
  currentIndex = (currentIndex + increment + WEB_TECH_IMAGES.length) % WEB_TECH_IMAGES.length;
}

//Обработчик для кнопнки Prev
